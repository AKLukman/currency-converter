...
function convertCurrency(currencyData) {
  let sourceCurrency = prompt('Enter the currency to convert from (source):').toLowerCase();
  if (!currencyData[sourceCurrency]) {
    displayErrorMessage('Invalid currency selected for Source');
  } else {
    let destinationCurrency = prompt('Enter the currency to convert to (destination):').toLowerCase();
